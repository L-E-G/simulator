...
impl ControlUnit {
    ...
    pub fn step(&mut self) -> Result<bool, String> {
        ...
        if self.empty_pipeline_count < 5 {
            //  Write back stage
        ...
        if self.empty_pipeline_count < 4 {
        // Access memory stage
        ...
        if self.empty_pipeline_count < 3 {
            // Execute stage
        ...
        if self.empty_pipeline_count < 2 {
            // Decode stage
                    ...
                    match InstructionT::match_val(itype) {
                        ...
                        Some(InstructionT::Control) => {
                            ...
                            match ControlOp::match_val(iop) {
                                Some(ControlOp::Halt) => {
                                    self.empty_pipeline_count = 1;
                                    self.empty_pipeline = true;
                                    Ok(Box::new(Halt::new()))
                                },
        ...
        if self.empty_pipeline_count < 1 {
            // Fetch stage
